...
class Politicos2012(Politicos):

    @classmethod
    def get_picture(cls, politician_id, city_cod):
        domain = 'http://divulgacand2012.tse.jus.br/divulgacand2012'
        action = 'mostrarFotoCandidato.action'
        return '{0}/{1}?sqCandidato={2}&codigoMunicipio={3}'.format(
            domain, action, politician_id, city_cod
        )
    ...
    def convert_to_dict(cls, data, state_siglum):
        item = dict(
            ...
            picture=cls.get_picture(cls.formatter(data[11]), data[6]),
            status=cls.formatter(data[42]),
            emails=cls.get_emails_in_text(data[43]),
        )
        return item
...
class Command(PoliticosCommand):
    ...
    def handle(self, *args, **options):
        Politicos2012.set_options(*args, **options)
        self.process_tse_data_by_year(Politicos2012, 2012)
